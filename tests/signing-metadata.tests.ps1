...
Assert-Match $install 'Start-Process -FilePath \$shortcut\.TargetPath' 'install.ps1 should start the resolved launcher target.'
...
Assert-Match $install 'Get-RandomInstallerPort' 'install.ps1 should select a random free port when no port is supplied.'
Assert-Match $install 'CODEX_PLUS_LAUNCH_PORT=' 'install.ps1 should return the launched port on stdout.'
...
Assert-Match $install '\$launcherArguments \+= \[string\]\$Port' 'install.ps1 should append the selected port to the launcher arguments.'
Assert-Match $install '\$env:CODEX_PLUS_REQUESTED_PORT = \[string\]\$Port' 'install.ps1 should pass the selected port to the launcher process environment as a fallback.'
Assert-Match $install 'Start-Sleep -Seconds 10' 'install.ps1 should keep the launcher port environment alive through asynchronous shortcut startup.'
