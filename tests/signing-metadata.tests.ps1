...
Assert-Match $install 'Launching Codex Plus from the desktop link\.' 'install.ps1 should launch Codex Plus from the desktop shortcut after install completes.'
Assert-Match $install 'Codex Plus desktop shortcut not found' 'install.ps1 should fail clearly when the desktop shortcut is missing.'
Assert-Match $install 'Start-Process -FilePath \$desktopShortcutPath' 'install.ps1 should start the desktop shortcut directly.'
