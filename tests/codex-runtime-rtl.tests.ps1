...
Assert-True ($payload.Contains('function applyTableAlignment')) 'Payload should align every cell in an RTL table to the right edge.'
Assert-True ($payload.Contains('data-codex-rtl-table-align')) 'Table alignment should be marked as tool-owned for cleanup.'
Assert-True ($payload.Contains("element.style.textAlign = 'right'")) 'RTL table cells should use explicit right alignment.'
Assert-True ($payload.Contains('applyTableAlignment(cell, tableDirection, direction)')) 'Table alignment should preserve each cell direction while aligning it.'
...
Assert-True ($sidebarPagingPayload.Contains('createSyntheticThreadRow(label, entry.lastModifiedMs, sourceRow)')) 'Synthetic Recents should clone a native row only when the source template does not already match.'
Assert-True ($sidebarPagingPayload.Contains('getNativeThreadTemplatesByThreadId')) 'Synthetic Recents should index native source rows by their own thread id.'
Assert-True ($sidebarPagingPayload.Contains('nativeThreadTemplatesByThreadId.get(threadId)')) 'Synthetic Recents should clone the matching native thread instead of the first available row.'
Assert-True ($sidebarPagingPayload.Contains('THREAD_SOURCE_TEMPLATE_ATTR')) 'Synthetic Recents should remember which native thread supplied each cloned row.'
Assert-True ($sidebarPagingPayload.Contains('existingRowHasMatchingSource')) 'Synthetic Recents should rebuild rows that were created from an old or unrelated template.'
Assert-True ($sidebarPagingPayload.Contains('removeSyntheticNativeStatusIcons')) 'Synthetic Recents should remove stale native cloud status icons from existing rows.'
...
Assert-True ($sidebarPagingPayload.Contains('Mark all as read')) 'Recents should expose a mark-all-as-read action.'
Assert-True ($sidebarPagingPayload.Contains('markConversationAsRead')) 'Mark-all-as-read should call Codex''s canonical conversation read API.'
Assert-True ($sidebarPagingPayload.Contains('setConversationUnreadState')) 'Mark-all-as-read should retain Codex''s mounted manager fallback.'
Assert-True ($sidebarPagingPayload.Contains('markAllRecentThreadsRead')) 'Recents should mark every rendered recent thread through Codex state.'
Assert-True ($sidebarPagingPayload.Contains('aria-busy')) 'Mark-all-as-read should expose its in-progress state accessibly.'
...
Assert-True ($sidebarPagingPayload.Contains('A real pointer activation emits both pointerup and click.')) 'Synthetic Threads should document the duplicate pointer/click activation guard.'
Assert-True ($sidebarPagingPayload.Contains("      stopSyntheticEvent();`n      // A real pointer activation emits both pointerup and click.")) 'Synthetic Threads should stop duplicate click propagation before checking navigation pending state.'
...
Assert-True (-not $sidebarPagingPayload.Contains('manager.activateThreadSummary(threadId)')) 'Synthetic Threads should not activate the native summary and expand its source project.'
Assert-True ($sidebarPagingPayload.Contains('suppressProjectThreads(sourceProjectId)')) 'Synthetic navigation should hide project-thread descendants before hydration can expand a closed source project.'
Assert-True ($sidebarPagingPayload.Contains('stabilizeSuppressedProjectState(sourceProjectId, sourceProjectWasClosed)')) 'Synthetic navigation should return late native project expansion state to closed while its descendants remain hidden.'
Assert-True ($sidebarPagingPayload.Contains('if (!projectClosedSuppressedThreadIds.has(normalizedProjectId)) return;')) 'A direct user project expansion should cancel the synthetic-navigation collapse stabilizer.'
Assert-True ($sidebarPagingPayload.Contains('data-codex-plus-project-threads-hidden')) 'Closed source projects should expose a stable project-thread visibility marker.'
Assert-True ($sidebarPagingPayload.Contains('[data-app-action-sidebar-project-list-id]')) 'The project-thread visibility guard should target the complete native child-thread list.'
Assert-True ($sidebarPagingPayload.Contains('PROJECTS_SECTION_COLLAPSED_ATTR')) 'Project threads should also stay hidden while the Projects section is collapsed.'
Assert-True ($sidebarPagingPayload.Contains('const projectsSectionWasClosed = Boolean(')) 'Synthetic navigation should capture a collapsed Projects section even when project rows are unmounted.'
Assert-True ($sidebarPagingPayload.Contains('suppressProjectsSectionForNavigation(sourceProjectId)')) 'Synthetic navigation should visually preserve a collapsed Projects section before hydration.'
Assert-True ($sidebarPagingPayload.Contains('stabilizeSuppressedProjectsSection(projectsSectionWasClosed)')) 'Synthetic navigation should return late native Projects-section expansion state to collapsed.'
Assert-True ($sidebarPagingPayload.Contains('data-codex-plus-projects-list-hidden')) 'A navigation-suppressed Projects list should remain hidden before React paints it.'
Assert-True ($sidebarPagingPayload.Contains('projectsSectionSuppressedThreadIds.clear()')) 'A direct user Projects-section expansion should release navigation-only thread suppression.'
...
Assert-True ($sidebarPagingPayload.Contains('hydrateThreadForNavigation')) 'Synthetic navigation should hydrate full conversation data before changing the route.'
Assert-True ($sidebarPagingPayload.Contains("'thread navigation hydration'")) 'Synthetic navigation hydration should use a bounded phase label.'
Assert-True ($sidebarPagingPayload.Contains('Thread navigation unavailable')) 'Synthetic navigation should report a direct-navigation failure instead of clicking another row.'
Assert-True (-not ($sidebarPagingPayload.Contains('const sourceRow = findSourceRow(row.getAttribute(SOURCE_LIST_ATTR)'))) 'Synthetic navigation should not proxy a Recents click to its project source row.'
...
Assert-True ($sidebarPagingPayload.Contains('startupThreadPreloadPromises')) 'Synthetic thread preload state should remain available for diagnostics.'
Assert-True ($sidebarPagingPayload.Contains('const THREAD_PRELOAD_ENABLED = true;')) 'Synthetic Recents should preload visible unselected threads.'
Assert-True ($sidebarPagingPayload.Contains('if (THREAD_PRELOAD_ENABLED) window.setTimeout(preloadStartupThreads, 0);')) 'Synthetic thread preloading should run at every sidebar refresh.'
...
Assert-True ($sidebarPagingPayload.Contains("if (THREAD_PRELOAD_ENABLED && sectionKey === 'threads')")) 'Show more should preload newly revealed synthetic threads.'
...
Assert-True ($sidebarPagingPayload.Contains("if (THREAD_PRELOAD_ENABLED && sectionKey === 'threads')")) 'The Threads Show more action should hydrate newly revealed rows.'
...
Assert-True ($sidebarPagingPayload.Contains('projectThreadVisibilityObserver')) 'Project-thread hiding should run before React can paint a newly mounted child list.'
...
Assert-True ($sidebarPagingPayload.Contains("title?.closest('[data-app-action-sidebar-section]')")) 'Synthetic Recents should be inserted outside the native Projects section so the toggles remain independent.'
Assert-True ($sidebarPagingPayload.Contains('already-rendered Recents section independent during that gap')) 'Synthetic Recents should survive a transient Projects-heading unmount.'
Assert-True ($sidebarPagingPayload.Contains('Do not erase known Recents rows')) 'Synthetic Recents should survive a temporary empty live catalog while projects are collapsed or threads are running.'
...
Assert-True ($newChatPayload.Contains('codex-plus-composer-utility-bars-v5')) 'Composer context persistence should use a new storage version after changing snapshot structure.'
...
Assert-True ($newChatPayload.Contains('button[title="Switch branch"]')) 'Persistent composer branch should read the live existing-thread branch control.'
Assert-True ($newChatPayload.Contains('data-codex-plus-branch-value')) 'Persistent composer snapshots should mark the exact branch value for live updates.'
Assert-True ($newChatPayload.Contains('syncPersistentBranchLabels')) 'Persistent composer branch should refresh after existing-thread metadata appears.'
...
Assert-True ($managerSource.Contains('$Instance.Payload = Get-CodexPlusPayloadBundle')) 'Runtime manager should refresh the payload before injecting new targets.'
Assert-True ($managerSource.Contains('@($changedFiles)') -and $managerSource.Contains('@($recentFiles)')) 'Usage refresh should merge FileInfo collections through a pipeline-safe array construction.'
...
Assert-True (-not $contextBadgePayload.Contains('codex-plus-context-badge-hide-style')) 'Context badge should not install a global hide style.'
Assert-True (-not $contextBadgePayload.Contains("style.setProperty('display', 'none', 'important')")) 'Context badge should remain visible while preserving its runtime state.'
Assert-True ($contextBadgePayload.Contains('codex-plus-open-elsewhere-hide-style')) 'Open-in-another-app alerts should have a persistent hide style.'
Assert-True ($contextBadgePayload.Contains('This is open in another app')) 'Open-in-another-app alerts should be identified by their exact title.'
Assert-True ($contextBadgePayload.Contains('data-codex-plus-hide-open-alert')) 'Open-in-another-app alerts should be hidden when rendered by the app.'
