...
Assert-True (-not ($sidebarPagingPayload.Contains('startupThreadPreloadPromises'))) 'Startup should not activate multiple conversations in the background.'
Assert-True (-not ($sidebarPagingPayload.Contains('THREAD_PRELOAD_SPINNER_ATTR'))) 'Module warmup should not pretend that conversations are already preloaded.'
