...
Assert-True ($newChatPayload.Contains('codex-plus-composer-utility-bars-v1')) 'Composer context persistence should retain captured rows across navigation in the current app session.'
...
Assert-True ($newChatPayload.Contains("snapshot.setAttribute('inert', '')")) 'Copied composer context rows should be informational rather than expose disconnected controls.'
