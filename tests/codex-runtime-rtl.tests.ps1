...
Assert-True ($sidebarPagingPayload.Contains('warmStartupThreadNavigation')) 'Startup should warm the private thread-navigation modules before the first click.'
Assert-True ($sidebarPagingPayload.Contains('void getInternalNavigationModules()')) 'Navigation warmup should import modules without waiting on a user click.'
...
Assert-True ($sidebarPagingPayload.Contains("indicator.style.left = '-24px'")) 'Preload indicators should remain in the dedicated gutter left of the synthetic thread text.'
Assert-True ($sidebarPagingPayload.Contains("button.classList.add('pl-6')")) 'Synthetic thread rows should retain their left inset for the indicator.'
Assert-True ($sidebarPagingPayload.Contains("button.style.paddingLeft = '24px'")) 'Synthetic thread rows should enforce the left inset when Tailwind class order overrides pl-6.'
Assert-True ($sidebarPagingPayload.Contains('applySyntheticThreadIndent(row);')) 'Synthetic thread rows should restore the left inset when existing rows are reused.'
...
Assert-True ($sidebarPagingPayload.Contains('startupThreadPreloadResults')) 'Full thread preload results should be cached for the next navigation.'
Assert-True ($sidebarPagingPayload.Contains("startupThreadOriginalReadThread(threadId, { includeTurns: true })")) 'Startup preloading should fetch the complete thread instead of only activating its summary.'
Assert-True ($sidebarPagingPayload.Contains('installThreadPreloadReadCache')) 'Thread navigation should consume the successful preload result.'
Assert-True ($sidebarPagingPayload.Contains('cloneThreadPreloadResult')) 'Cached thread reads should return independent objects because Codex mutates metadata responses.'
...
Assert-True ($sidebarPagingPayload.Contains('getThreadResumeFunction')) 'Startup preloading should resolve Codex''s private conversation hydration function structurally.'
...
Assert-True ($sidebarPagingPayload.Contains('await resumeThread(hydrationManager')) 'Startup preloading should hydrate Codex conversation state with the full app-server manager before showing completion.'
Assert-True ($sidebarPagingPayload.Contains('hydrationManager.getCompleteConversationTurns(threadId)')) 'Startup preloading should validate the canonical turn history consumed by the thread view.'
Assert-True ($sidebarPagingPayload.Contains('const manager = getThreadHydrationManager(modules, scope)')) 'Synthetic navigation should use the fully hydrated manager instead of falling back through a closed project.'
Assert-True ($sidebarPagingPayload.Contains("hydratedConversation?.resumeState !== 'resumed'")) 'Preload completion should require a resumed conversation state.'
Assert-True ($sidebarPagingPayload.Contains('hydratedTurns.length === 0')) 'Preload completion should require canonical rendered turns when the fetched thread has turns.'
Assert-True ($sidebarPagingPayload.Contains('THREAD_PRELOAD_MIN_DISPLAY_MS = 750')) 'Preload spinners should remain visible long enough to be observed.'
...
Assert-True ($sidebarPagingPayload.Contains("hostId + ':' + threadId")) 'Synthetic Threads should navigate using Codex''s native local thread key format.'
Assert-True ($sidebarPagingPayload.Contains("return 'flat-chats'")) 'Synthetic task threads should use Codex''s flat chats location.'
Assert-True ($sidebarPagingPayload.Contains("return 'project:' + projectId")) 'Synthetic project threads should use Codex''s project location.'
